...
public class MediaServerClient {
    ...
    public byte[] downloadRecordingFile(String recordingId) {
        try {
            logger.info("Downloading recording file from media server: {}", recordingId);

            byte[] fileData = webClient.get()
                    .uri("/recordings/{recordingId}", recordingId)
                    .retrieve()
            ...
            if (fileData == null || fileData.length == 0) {
                throw new RuntimeException("Empty recording file received");
            }

            logger.info("Downloaded recording file: {} ({} bytes)", recordingId, fileData.length);
            return fileData;

        } catch (Exception e) {
            logger.error("Error downloading recording file {}: {}", recordingId, e.getMessage(), e);
            throw new RuntimeException("Failed to download recording file: " + e.getMessage(), e);
        }
    }
